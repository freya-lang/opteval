...
pub(crate) fn interact(left: &Port, right: &Port) {
	...
	let (left_new, right_new) = match (left.node().data(), right.node().data()) {
		...
		(&Data::Ascend { .. }, &Data::Binding { index }) => ascend_binding(index),
		(&Data::Binding { index }, &Data::Ascend { .. }) => mirror(ascend_binding(index)),
...
fn ascend_binding(index: usize) -> (Vec<Port>, Vec<Port>) {
	let left_anchor = anchor();
